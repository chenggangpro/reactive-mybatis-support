...
* ⚠️ When using `useGeneratedKeys="true"` in `insert` sql, the `keyColumn="xxxx"` must be set
  * 1 . The `JDBC driver`'s`Connection` provides a `PreparedStatement prepareStatement(String sql, int autoGeneratedKeys) throws SQLException;` method, the second parameter of this method indicates whether auto returns generatedKeys
