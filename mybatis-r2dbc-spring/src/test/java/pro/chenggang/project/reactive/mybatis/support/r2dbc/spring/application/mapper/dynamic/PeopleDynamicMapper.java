...
public interface PeopleDynamicMapper extends CommonSelectMapper {
    ...
    default Mono<Long> count(CountDSLCompleter completer) {
        return ReactiveMyBatis3Utils.countFrom(this::count, people, completer);
    }

    default Mono<Integer> delete(DeleteDSLCompleter completer) {
        return ReactiveMyBatis3Utils.deleteFrom(this::delete, people, completer);
    }

    default Mono<Integer> insert(People record) {
        return ReactiveMyBatis3Utils.insert(this::insert, record, people, c ->
    ...
    default Mono<Integer> insertMultiple(Collection<People> records) {
        return ReactiveMyBatis3Utils.insertMultiple(this::insertMultiple, records, people, c ->
            c.map(id).toProperty("id")
            .map(nick).toProperty("nick")
            .map(createdAt).toProperty("createdAt")
        );
    ...
    default Mono<People> selectOne(SelectDSLCompleter completer) {
        return ReactiveMyBatis3Utils.selectOne(this::selectOne, selectList, people, completer);
    }

    default Flux<People> select(SelectDSLCompleter completer) {
        return ReactiveMyBatis3Utils.selectList(this::selectMany, selectList, people, completer);
    }

    default Flux<People> selectDistinct(SelectDSLCompleter completer) {
        return ReactiveMyBatis3Utils.selectDistinct(this::selectMany, selectList, people, completer);
    }

    default Mono<Integer> update(UpdateDSLCompleter completer) {
        return ReactiveMyBatis3Utils.update(this::update, people, completer);
    }
